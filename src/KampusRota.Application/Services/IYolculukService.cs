...
namespace KampusRota.Application.Services
{
    ...
    {
        Task<IEnumerable<Yolculuk>> TumYolculuklariGetirAsync(int? universityId = null);

        Task<IEnumerable<Yolculuk>> MusaitYolculuklariGetirAsync(string kalkisNoktasi, string varisNoktasi, DateTime tarih, int? universityId = null);

        Task<Yolculuk?> YolculukGetirByIdAsync(int id);
